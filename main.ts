    ...
    async fetchOpenAIModels(apiKey: string): Promise<FetchedModelInfo[]> {
        ...
        try {
            ...
            if (data && Array.isArray(data.data)) {
                ...
                // Persist discovered IDs so the dropdown survives this.display().
    ...
    async fetchGoogleModels(apiKey: string): Promise<FetchedModelInfo[]> {
        ...
        try {
            ...
            if (data && Array.isArray(data.models)) {
                ...
                // captured) so the dropdown survives this.display().
                this.settings.fetchedModels = {
                    ...this.settings.fetchedModels,
...
class YouTubeTranscriptModal extends Modal {
    ...
    private buildInputStage() {
        ...
        toggleLabel.createDiv({ text: 'Fast summary mode' });
...
class YouTubeTranscriptSettingTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        // are split out into createProviderConfigBlock and only shown for the
        // selected provider (so the LLM section reflects the active provider's
        // context rather than rendering all four providers' full settings at once).
        const createProviderApiKeyRow = (
            provider: string,
            displayName: string,
            placeholder: string,
        ): void => {
            new Setting(settingsContainer)
                ...
                .addText(text => {
                    ...
                    if (inputEl) {
                        inputEl.type = 'password';
                        inputEl.addEventListener('focus', () => { inputEl.type = 'text'; });
                        inputEl.addEventListener('blur', () => { inputEl.type = 'password'; });
                    }
                    return textComponent;
                });
        };
        ...
        // and the custom-model-parameters section (which has its own visibility
        // gating based on whether a custom model is selected).
        const createProviderConfigBlock = (
            provider: string,
            displayName: string,
            modelOptions: string[],
            ...
            new Setting(settingsContainer)
                ...
                .setDesc('Select a preset model or type a custom model name below.')
            ...
            // (whose API doesn't return limits) survives this.display() the
            // same way Google and Anthropic do. Deduplicated and sorted.
            const fetched = this.plugin.settings.fetchedModels ?? {};
            ...
            setting.addDropdown(dropdown => {
                ...
                dropdown.setValue(validSelection)
                    .onChange((value: string) => {
                        ...
                        // below addCustomModelParametersSection). The listener
                        // calls paramsHandle.refresh() which reads
            ...
            setting.addText(text => {
                ...
                text.setPlaceholder('Custom model name')
                    .setValue(
                        ...
                        !mergedOptions.includes(this.plugin.settings.selectedModels[provider])
            ...
            // Add the always-visible model parameters section. Captures a
            // refresh handle we wire into the dropdown's onChange below so a
            // model switch immediately repopulates the override fields with
            // the new model's resolved limits (registry default or saved
            // override). Without this the panel would keep showing the
            // previous model's values until the user touched another control.
            //
            // Note on the local aliases: `modelDropdown` and `customField` are
            // mutated inside addDropdown / addText callbacks; TypeScript's
            // closure-narrowing leaves them typed as `null` here. The aliases
            // assert non-null after the runtime guard so we can use them
            // without re-narrowing on every line.
            const dropdownRef = modelDropdown as DropdownComponent | null;
            const fieldRef = customField as TextComponent | null;
            if (fieldRef && dropdownRef) {
        ...
        // Always render an API key row for every provider — users can
        // populate or update any key irrespective of which LLM is currently
        // selected.
        for (const entry of providerCatalog) {
            createProviderApiKeyRow(entry.provider, entry.displayName, entry.placeholder);
        }

        // Render the model config block for the currently-selected provider only.
        // Switching the provider re-runs display() (the Select-LLM dropdown's
        // onChange calls this.display()), which re-enters this code path and
        // renders the new provider's block.
        const selectedEntry = providerCatalog.find(e => e.provider === this.plugin.settings.selectedLLM);
        if (selectedEntry) {
            createProviderConfigBlock(
                selectedEntry.provider,
                selectedEntry.displayName,
                selectedEntry.modelOptions,
                selectedEntry.defaultModelValue,
            );
        }
     ...
     */
    private addCustomModelParametersSection(container: HTMLElement, provider: string, customField: TextComponent, modelDropdown: DropdownComponent): { refresh: () => void } {
        // Create container — always visible for the selected provider.
        const customParamsContainer = container.createDiv({
            cls: 'tubesage-custom-model-params'
        });

        // Add header (text updated by updateCustomParamsLabel below).
        const customParamsHeading = new Setting(customParamsContainer)
            .setName(`Model parameters (${provider.toUpperCase()})`)
            .setHeading();
        customParamsHeading.settingEl.addClass('tubesage-custom-params-header');

        // Optional sub-description shown under the heading. Updated alongside
        // the heading text by updateCustomParamsLabel.
        const subDescEl = customParamsContainer.createDiv({
            cls: 'setting-item-description tubesage-custom-params-subdesc',
        });
        ...
        const getCurrentCustomLimits = () => {
            const model = this.plugin.settings.selectedModels[provider];
            const customKey = `${provider}:${model}`;
            const userOverride = this.plugin.settings.customModelLimits[customKey];
        ...
        // Hold field references so we can refresh their displayed values when
        // the model selection changes (or a refresh repopulates the registry).
        // setValue is called once at construction; without these refs the
        // panel keeps showing the previous model's values.
        let contextKField: TextComponent | null = null;
        let maxOutputKField: TextComponent | null = null;
        let inputMaxKField: TextComponent | null = null;
        let reservePctField: TextComponent | null = null;
        ...
        // The container is always visible. We update the heading text and
        ...
        const headingNameEl = customParamsHeading.settingEl.querySelector('.setting-item-name');
        const updateCustomParamsLabel = (): void => {
            const isCustomSelected = modelDropdown.getValue() === 'custom';
            if (headingNameEl && headingNameEl.instanceOf(HTMLElement)) {
                headingNameEl.setText(
                    isCustomSelected
        ...
        const refreshFieldValues = (): void => {
            ...
            contextKField?.setValue(limits.contextK.toString());
            maxOutputKField?.setValue(limits.maxOutputK.toString());
            inputMaxKField?.setValue(limits.inputMaxK ? limits.inputMaxK.toString() : '');
            reservePctField?.setValue(
                limits.reservePct?.toString() ?? (provider === 'ollama' ? '0.15' : '0.10'),
            );
        };

        // Combined refresh: re-apply both label and field values. Returned
        // to the caller so the dropdown's onChange handler can invoke it
        // synchronously (after writing selectedModels) — that's strictly
        // ordered, unlike a separate DOM-event listener which runs in a
        // race against Obsidian's async onChange wrapper.
...
class TemplateFilePickerModal extends Modal {
    ...
    updateTemplateList(query: string, listEl?: HTMLElement) {
        ...
        if (filteredTemplates.length === 0) {
            templateListEl.createDiv({ text: 'No matching templates found.' });
        }
    }
            ...
            for (const line of lines) {
                ...
                // Handle list items
                    ...
                    if (match) {
                        ...
                        listItem.createSpan({ text: `${number}. `, cls: 'tubesage-license-list-item-title-segment' });
                        
                        listItem.createSpan({ text: `${title}: `, cls: 'tubesage-license-list-item-title-segment' });
