...
import { planCancel, planColdStartClosure } from "./collection-policy";
...
const parentOf = (n: number, installationId = "inst-1") => ({
  ...planCollection({
    url: "u", folder: "f", sourceName: "s", contentType: "Playlist",
    installationId, createdAt: 0, id: "p", plannedCount: n,
  }),
...
describe("planColdStartClosure — a run dies with the app instance", () => {
  it("closes this installation's unfinished run and does NOT resume its children", () => {
    const plan = planColdStartClosure(parentOf(3), [
      child("c1", "done"), child("c2", "running"), child("c3", "interrupted"),
    ], "inst-1");
    expect(plan.close).toBe(true);
    expect(plan).not.toHaveProperty("resumeIds");
    expect(plan.parentStatus).toBe("closed");
  });

  it("reports what the run left behind, so the notice can be honest", () => {
    const plan = planColdStartClosure(parentOf(3), [
      child("c1", "done"), child("c2", "done"), child("c3", "running"),
    ], "inst-1");
    expect(plan.progress).toMatchObject({ total: 3, done: 2 });
  });

  it("ignores a run owned by another installation, which may be live on that device", () => {
    const plan = planColdStartClosure(parentOf(2, "other-install"), [child("c1", "running")], "inst-1");
    expect(plan.close).toBe(false);
  });

  it("leaves an already-finished run alone", () => {
    const parent = { ...parentOf(1), status: "done" as const };
    expect(planColdStartClosure(parent, [child("c1", "done")], "inst-1").close).toBe(false);
  });
});
